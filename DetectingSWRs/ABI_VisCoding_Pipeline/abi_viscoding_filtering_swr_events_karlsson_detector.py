...
def get_session_id_numbers_from_swr_event_directories(directory_path):
    ...
    all_directories = [
        ...
        if os.path.isdir(os.path.join(directory_path, d)) and "swrs_session_" in d
    ]
...
def get_numbers_between_substrings(directory_path, substring1, substring2):
    ...
    for filename in all_files:
        if (
            ...
            and substring1 in filename
            and substring2 in filename
        ):
...
if len(select_these_sessions) == 0:
    select_these_sessions = get_session_id_numbers_from_swr_event_directories(
        input_dir_filter
    )
...
for seshnum in tqdm(
    range(0, len(select_these_sessions)), desc="Processing", unit="iteration"
):
    ...
    # make probe list
    probe_file_list = get_files_with_substrings(
        session_path, substring1="probe", substring2="karlsson_detector_events"
    )
    ...
    for probe_num in range(0, len(probe_file_list)):
        ...
        # check if they overlap with gamma events
        gamma_event_file = get_files_with_substrings(
            session_path,
            substring1="probe_" + str(probe_id),
            substring2="gamma_band_events",
        )
        gamma_events = pd.read_csv(
            os.path.join(session_path, gamma_event_file[0]),
            index_col=0,
            compression="gzip",
        )
        putative_ripples_df["Overlaps_with_gamma"] = check_overlap(
            putative_ripples_df, gamma_events
        )
        print("Gamma events")
        ...
        movement_channels_files = get_files_with_substrings(
            session_path,
            substring1="probe_" + str(probe_id),
            substring2="movement_artifacts",
        )
        movement_channel_1 = pd.read_csv(
            os.path.join(session_path, movement_channels_files[0]), compression="gzip"
        )
        movement_channel_2 = pd.read_csv(
            os.path.join(session_path, movement_channels_files[1]), compression="gzip"
        )
        overlapping_artifacts = []
        if movement_channel_1.shape[0] > movement_channel_2.shape[0]:
            overlapping_artifacts = movement_channel_1[
                check_overlap(movement_channel_1, movement_channel_2)
            ]
        elif movement_channel_1.shape[0] < movement_channel_2.shape[0]:
            overlapping_artifacts = movement_channel_2[
                check_overlap(movement_channel_2, movement_channel_1)
            ]
        print("Check for overlapping movement artifacts")
        putative_ripples_df["Overlaps_with_movement"] = check_overlap(
            putative_ripples_df, overlapping_artifacts
        )

        print("Writing events with fitlers to file")
        putative_ripples_df.reset_index(drop=True).to_csv(
            events_csv_path, index=False, compression="gzip"
        )
        num_rips = (
            (putative_ripples_df["Overlaps_with_gamma"] == False)
            & (
                putative_ripples_df["Overlaps_with_movement"]
                == include_gamma_intersection
            )
        ).sum()
        new_row = {
            "session_id": session_id,
            "probe_id": probe_id,
            "ripple_number": num_rips,
        }
        eventspersession_df = pd.concat(
            [eventspersession_df, pd.DataFrame([new_row])], ignore_index=True
        )
...
eventspersession_df.to_csv(
    os.path.join(swr_output_dir_path, "eventspersession_df.csv"),
    index=False,
    compression="gzip",
)
