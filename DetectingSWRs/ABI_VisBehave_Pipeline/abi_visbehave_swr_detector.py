...
def process_session(session_id):
    ...
    try:
        ...
        probe_id_list = list(
            valid_lfp[valid_lfp.ecephys_session_id == session_id].index
        )
        del probes_table_df
        del valid_lfp
        probes_of_interest = []
...
with Pool(pool_size, initializer=init_pool, initargs=(queue,)) as p:
    ...
    p.map(process_session, sessions_with_CA1)
...
command_empty_swrdir_clear = (
    f"find {swr_output_dir_path} -mindepth 1 -type d -empty -delete"
)
if save_lfp == True:
    command_empty_lfpdir_clear = (
        f"find {lfp_output_dir_path} -mindepth 1 -type d -empty -delete"
    )
