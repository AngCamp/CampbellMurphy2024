...
def process_session(session_id):
    ...
    try:
        ...
        for this_probe in range(len(probelist)):
            ...
            process_stage = (
                f"Getting channels data for probe {probe_name}, with id : {probe_id}"
            )
            ...
            if "CA1" not in regions_on_probe:
                ...
                continue
            process_stage = (
                f"Getting LFP data for the probe {probe_name} with id {probe_id}"
            )
            # Find the relevant datasets and download them
            dsets = one.list_datasets(
                eid, collection=f"raw_ephys_data/{probe_name}", filename="*.lf.*"
            )
            print(type(dsets))
            print(len(dsets))
            ...
            lfp_time_index_og = (
                np.arange(0, sr.shape[0]) * dt + t0
            )  # get the time index for the LFP data
            del ssl
            ...
            process_stage = (
                f"Destriping the LFP data for probe {probe_name} with id {probe_id}"
            )
            start_time = time.time()
            ...
            lfp_ca1, lfp_time_index = resample_signal(
                lfp_ca1, lfp_time_index_og, 1500.0
            )  # note the original samplig rate is infered from the times object
            lfp_ca1 = (
                lfp_ca1.T
            )  # transpose to get the right shape for the rest of the code
            ...
            this_chan_id = "channelsrawInd_" + str(
                ca1_chans[highest_rip_power.argmax()]
            )  # for naming the file later, this way we know what object its from so we dont go looking for an id
            ...
            take_two = np.random.choice(
                channels.rawInd[not_a_ca1_chan], 2, replace=False
            )
            outof_hp_chans_lfp = []
