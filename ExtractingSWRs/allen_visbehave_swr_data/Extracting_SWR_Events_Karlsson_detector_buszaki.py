...
swr_output_dir = 'allen_visbehave_swr_5sdthresh' # directory specifying the output
...
ripple_band_threshold = 5
movement_artifact_ripple_band_threshold = 5
...
for seshnum in tqdm(range(0, sessions.shape[0]), desc="Processing", unit="iteration"):
    ...
    for probe_id in probes_of_interest:    
        ...
        lfp_time_index = lfp.time.values
        ...
        gamma_band = finitimpresp_filter_for_LFP(lfp_ca1[:,lfp_ca1_chans == this_chan_id], samplingfreq =  sampling_rate_this_probe, lowcut = 20, highcut = 80)
