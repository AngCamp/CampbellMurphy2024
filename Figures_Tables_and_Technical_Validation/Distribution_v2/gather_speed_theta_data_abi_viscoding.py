...
# IMPORTANT: This script is ONLY for ABI Visual Coding
# Uses EcephysProjectCache and /allen_viscoding_data
...
ABI_CODING_CACHE = "/space/scratch/allen_viscoding_data"

THETA_HALF_WINDOW = 0.125  # seconds

# Argument parsing
def parse_args():
    parser = argparse.ArgumentParser(description="Gather mean theta power and speed for SWR events (ABI Visual Coding only).")
...
def main():
    ...
    # Set up the cache
    manifest_path = os.path.join(ABI_CODING_CACHE, "manifest.json")
    cache = EcephysProjectCache.from_warehouse(manifest=manifest_path)
    # Data folders
    swr_dir = os.path.join(DATA_ROOT, "allen_viscoding_swr_murphylab2024")
    lfp_dir = os.path.join(DATA_ROOT, "allen_viscoding_swr_murphylab2024_lfp_data")
    ...
    for session in session_dirs:
        ...
        session_obj = cache.get_session_data(session_id)
