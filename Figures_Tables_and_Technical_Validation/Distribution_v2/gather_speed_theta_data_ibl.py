...
# IMPORTANT: This script is ONLY for IBL data
...
# Argument parsing
def parse_args():
    parser = argparse.ArgumentParser(description="Gather mean theta power and speed for SWR events (IBL only).")
...
def main():
    ...
    # Data folders
    swr_dir = os.path.join(DATA_ROOT, "ibl_swr_murphylab2024")
    lfp_dir = os.path.join(DATA_ROOT, "ibl_swr_murphylab2024_lfp_data")
    ...
    for session in session_dirs:
        ...
        # Load wheel speed data from ONE API
        try:
            ...
            print(f"    Error loading wheel data for session {session_id}: {e}")
            continue
