...
abi_ripples_path =  '/space/scratch/allen_visbehave_swr_data/allen_visbehave_swr_murphylab2024'
...
def process_speed_and_theta(sessionfolder, abi_ripples_path, abi_lfp_path, sdk_cache_dir, theta_filter, theta_compute_halfwindow):
    try:
        ...
        for _, row in global_ripples_df.iterrows():
            ...
            if mask.size > 0 and not np.isnan(wheel_velocity[mask]).all():
                ...
                average_speed = 0  # or np.nan, depending on how you want to handle this case
            average_speed = np.abs(wheel_velocity[mask]).mean()
            average_speeds.append(average_speed)
        
        theta_powers = {'peakwindow_med': [], 'peakwindow_mean': [], 'eventwindow_med': [], 'eventwindow_mean': []}
        ...
        for file in folderfiles:
            ...
            lfp_data_file = [f for f in lfp_files if f"channel_{channel_indx}" in f and probe_id in f and "ca1_peakripplepower.npz" in f][0]
            lfp_data = np.load(os.path.join(lfp_session_path, lfp_data_file))['lfp_ca1']
            
            lfp_times_file = [f for f in lfp_files if f"channel_{channel_indx}" in f and probe_id in f and "time_index_1500hz.npz" in f][0]
            lfp_times = np.load(os.path.join(lfp_session_path, lfp_times_file))['lfp_time_index']
            
            theta_pow_zscore = np.convolve(lfp_data.flatten(), theta_filter.flatten(), mode='same')
            theta_pow_zscore = scipy.stats.zscore(np.abs(hilbert(theta_pow_zscore))**2)
...
def worker_function(shared_speeds, shared_theta_powers, sessionfolder, abi_ripples_path, abi_lfp_path, cache, theta_filter, theta_compute_halfwindow):
    # Call the process_speed_and_theta function to get data
    average_speeds, theta_powers = process_speed_and_theta(sessionfolder, abi_ripples_path, abi_lfp_path, cache, theta_filter, theta_compute_halfwindow)
...
if __name__ == '__main__':
    ...
    sessionfolders = [folder for folder in os.listdir(abi_ripples_path) if not '.csv' in folder] # Adjust as needed
    ...
    with Manager() as manager:
        ...
        args_list = [(shared_speeds, shared_theta_powers, folder, abi_ripples_path, abi_lfp_path, sdk_cache_dir, theta_filter, theta_compute_halfwindow) for folder in sessionfolders]
        pool_size = 10
